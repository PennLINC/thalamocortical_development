...
gam.fit.smooth <- function(measure, atlas, dataset, region, smooth_var, covariates, knots, set_fx = FALSE){
...
#Create a prediction data frame, used to estimate (posterior) model slopes (varying covariate coefficients)
  ...
  for (v in c(1:length(theseVars))) {
    ...
    if (thisVar == int_var) { 
      ...
      pred.low[,smooth_var] = seq(8, 22, length.out = np)
    } else {
      switch (thisClass,
  ...
  for (v in c(1:length(theseVars))) {
    ...
    if (thisVar == int_var) { 
      ...
      pred.high[,smooth_var] = seq(8, 22, length.out = np)
    } else {
      switch (thisClass,
